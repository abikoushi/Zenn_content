...
rep_sim_branch <- function(iter,R0,k,G){
  ...
  for( i in 1:iter){
    ...
    if(length(tree)>0){
      tree$group <- i
      res[[i]] <- tree
      total_size[i] <- nrow(tree) + 1L #最初の1人分足す
    }else{
      res[[i]] <- data.frame(id=NA_integer_,
                             parent=NA_integer_,
                             g=NA_integer_,
                             group=i)
      total_size[i] <- 1L #最初の1人分
    }
  }
  res <- bind_rows(res)
...
print(max(res$tree$g, na.rm = TRUE))

tab <- table(res$total_size)
ran <- range(as.integer(names(tab)))
p <- sapply(ran[1]:ran[2], prob_totalsize, R0=R0,k=k)
png("totalsize.png")
plot(tab/sum(tab), ylim = c(0, 0.6), xlab="Y", ylab="prob.")
points(ran[1]:ran[2], p)
dev.off()
