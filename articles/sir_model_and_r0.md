...
[西浦博　編著『感染症流行を読み解く数理』（日本評論社）](https://www.nippyo.co.jp/shop/book/8827.html) の第１章のはじめの方を，適当な数値例を自分なりに R で補いながら読んでいくものです．この第１章の筆者は小林鉄郎，西浦博です．ただし，（当たり前かもしれませんが，）この文書の責任は私にあります．
...
$S(t)$ は $t$ 時点でこれから感染する可能性のある人の数，$I(t)$ は感染者数，$R(t)$ は除外数——すなわち病気から回復して免疫を獲得した人などもう感染する可能性のない人の数に対応する． $\beta$, $\gamma$ はパラメータで，$\beta$ は感染者一人あたりかつ単位時間あたりの感染率，$\gamma$ は除外率である．今回は $S(t)+I(t)+R(t)=1$ になるよう正規化されているとする．

SIRモデルの解をプロットしてみよう．微分方程式を数値的に解くRのパッケージに `deSolve` がある．SIRモデルを数値的に解くには次のようにすればよい．
...
![](/images/sir_model_and_r0/SIR_sol.png)
*SIRモデルの解 $S(t), I(t), R(t)$ のグラフ．横の点線については後述する．*

パラメータは `pars  <- c(beta=1.5, gamma=1)` の部分で　$\beta=1.5, \gamma=1$ と与えた．

さて，感染症の流行時に感染が広がるのか収束していくのかには特に興味がある．そこで初期値をいくつか振り，感染者数 $I(t)$ をプロットしてみる．総和が1になるよう，初期値は次のように与えた．
...
![](/images/sir_model_and_r0/SIR_I.png)
*SIRモデルの解 $I(t)$ についてのグラフ．色分けは $S(0)$ による．*

初期値による色分けに着目すると，ある値を境に $I(t)$ が増加するか単調に減少するかが分かれることがわかる．実は，この「ある値」は $\gamma / \beta$ である．
...
いま，$S(0)$ がほぼ 1，$R(0)$ が0の状況を考え，左辺の $\beta/\gamma$ を基本再生産数と呼ぶ．以下では基本再生産数を $R_0 = \beta/\gamma$ とおく．記号が似ているが，この $R_0$ は $R(0)$ とは別のものである．
...
実はこれを $z$ について解いた値が最初の図に引いていた点線の意味である．数値的に解くために次のようにした．
...
いま $R(0)$ は0，$S(0)$ はほぼ1の状況を考えていたので，上の等式を次のように表すことができる．
...
![](/images/sir_model_and_r0/size_by_R0.png)
*基本再生産数ごとの最終的な感染者数*

このノートで使用したコードの全体を以下に置く．

https://github.com/abikoushi/Zenn_content/blob/main/R/sir_model_and_r0.R
