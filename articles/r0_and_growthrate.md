...
いま，SIRモデルに戻って，感染者はレート $\gamma$ で除外されていくとする．さらに，よく使われる正の連続型の分布として一番かんたんそうな指数分布を考え，感染者はレート $\gamma$ の指数分布に従って除外されるとする．

指数分布の分布関数を $F(\tau)$ と書くことにする．$A(\tau)$ は除外されずに残る確率に比例するとすると，

$$
A(\tau) \propto1-F(\tau)= \exp(-\gamma \tau).
$$
